...
INSTALL_RECEIPT="/usr/local/Cellar/emacs/(HEAD-*)/INSTALL_RECEIPT.json"

LAST_COMMIT_BUILT=$(cat $INSTALL_RECEIPT | python -c "import sys, json; print(json.load(sys.stdin)['HEAD']))")
